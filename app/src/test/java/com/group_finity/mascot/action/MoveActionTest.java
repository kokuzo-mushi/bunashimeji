...
class MoveActionTest {
    ...
    void setUp() {
        ...
        // マスコットの初期位置を設定
        when(mockMascot.getAnchor()).thenReturn(new NeoPoint(100, 100));
    }
    ...
    void execute_shouldMoveToTargetImmediately_whenDurationIsZero() {
        ...
        verify(mockMascot).setAnchor(new NeoPoint(200, 200));
        // アクションが即座に終了することを確認
    ...
    void execute_shouldMoveIncrementally_whenDurationIsPositive() {
        ...
        assertEquals(new NeoPoint(140, 140), pointCaptor.getValue());
        ...
        assertEquals(new NeoPoint(180, 180), pointCaptor.getValue());
        assertTrue(moveAction.hasNext(), "アクションはまだ継続中のはず");
    }
    ...
    void hasNext_shouldReturnFalse_afterDuration() {
        ...
        assertEquals(new NeoPoint(200, 200), pointCaptor.getAllValues().get(pointCaptor.getAllValues().size() - 1));
